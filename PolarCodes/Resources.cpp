...
unsigned char Crc8(int* data, int len)
{
    unsigned char crc = 0xFF;
    unsigned char curentCrc;
    ...
    for (int i = 0; i < len; i+=8) {
        ...
        crc = Crc8Table[crc ^ curentCrc];
    }
    return crc;
}

unsigned short Crc16(int* data, int len)
{
    ...
    for (int i = 0; i < len; i += 8) {
        ...
        crc = (crc << 8) ^ Crc16Table[(crc >> 8) ^ curentCrc];
    }
...
void ReverseShuffle(int n, int* a) {
    for (int i = 0; i < n; i++) {
        reverseShuffleMem[i] = i * 2 < n
            ? a[i * 2]
            : a[1 + i * 2 - n];
    }
    for (int i = 0; i < n; i++) {
